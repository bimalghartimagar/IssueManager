<template>
    <div v-if="ticket" class="rounded shadow mt-8 pb-1 border border-indigo-400">
        <div class="w-full text-left p-4 text-xl bg-indigo-400 shadow">Details of ticket #{{ticket.id}}</div>
        <div class="grid grid-cols-3 gap-4 shadow p-3">
            ...
            <div>
                Status:
                <span class="rounded-lg text-white px-4 py-1 text-sm capitalize" :class="util.getStatusColor(ticket.status)">{{ ticket.status }}</span>
            </div>
        </div>
